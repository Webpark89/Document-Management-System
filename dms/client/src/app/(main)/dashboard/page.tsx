...
export default function DashboardPage() {
  ...
  React.useEffect(() => {
    getDocuments().then(docs => {
      ...
      const mapped = docs.map(d => ({
        ...
        status: d.status,
        submittedBy: d.sender || d.creator_name,
  ...
  const typeData = useMemo(() => {
    const counts: Record<string, number> = { PR: 0, PO: 0, บันทึก: 0, Other: 0 };
    ...
    return [
      ...
      { name: "บันทึก", value: counts["บันทึก"], fill: TYPE_COLORS["บันทึก"] },
